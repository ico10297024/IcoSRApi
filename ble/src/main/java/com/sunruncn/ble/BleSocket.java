...
public class BleSocket {
     ...
     */
    public BleSocket(Context context, String filter, final int type) {
        this.keywordFilter = filter;
        BleHelper bleHelper = new BleHelper(context, new BleCallback() {
            @Override
        ...
        //设置过滤器
        bleHelper.setBleFilter(new BleHelper.BleFilter() {
            @Override
            public boolean onBleFilter(BluetoothDevice device) {
                switch (type) {
                    case 0:
                        return device.getAddress().replace(":", "").equalsIgnoreCase(keywordFilter);
                    case 1:
                        if (TextUtils.isEmpty(device.getName())) {
                            return false;
                        }
                        return device.getName().indexOf(keywordFilter) != -1;
                }
                return true;
            }
        });
     ...
     */
    public BleSocket(Context context, UUID[] servicesUUID) {
    ...
    /**
     * 同{@link BleSocket#BleSocket(Context, String, int)}
     *
     * @param context
     * @param bleHelper
     */
    public BleSocket(Context context, BleHelper bleHelper) {
        this.mContext = context;
        this.mBleHelper = bleHelper;
    }
...
//            log.w(String.format("%s,发送数据：%s；通道：%s", this.toString(), Common.bytes2Int16(data, " "), uuid), TAG);
            log.w(String.format("%s,发送数据", this.toString()), TAG);
    ...
    public void setReadCharacteristicNotification() {
        ...
        if (characteristic == null) {
            log.e("没有找到readUUID对应的通道，UUID：" + characteristic.getUuid(), TAG);
            return;
        }
        if (!canNotify(characteristic)) {
            log.e("通道没有通知的特性，UUID：" + characteristic.getUuid(), TAG);
            return;
        }
//        log.w("设置通道通知，UUID：" + characteristic.getUuid(), TAG);
        log.w("设置通道通知", TAG);
...
//                    log.d("BleSocket find characteristic：" + characteristic.getUuid().toString());
    ...
    public String toString() {
        ...
        } else if (!TextUtils.isEmpty(keywordFilter)) {
            return "keywordFilter:" + keywordFilter;
        } else {
            return null;
        }
    }
